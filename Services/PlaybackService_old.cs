...
public sealed class PlaybackService : IPlaybackService
{
        ...
        ILogger<PlaybackService> logger)
    {
        ...
        _localPlayer = _players.TryGetValue(PlaybackOutputMode.Local, out var local)
            ? local as ILocalPlayerService
            : null;
        _sendspinPlayer = _players.TryGetValue(PlaybackOutputMode.Sendspin, out var sendspinPlayer)
            ? sendspinPlayer as SendspinPlayerService
            : null;

        _activePlayer = _players.TryGetValue(PlaybackOutputMode.Sendspin, out var sendspin)
            ? sendspin
    ...
    public async Task SetOutputModeAsync(PlaybackOutputMode mode, string? targetPlayerId = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            PlaybackOutputMode.Local => _players[PlaybackOutputMode.Local],
            PlaybackOutputMode.MA_Remote => _players[PlaybackOutputMode.MA_Remote],
            _ => _players[PlaybackOutputMode.Sendspin]
        };
        ...
        // When leaving Sendspin: reset hybrid state and disable Sendspin external source.
        if (mode != PlaybackOutputMode.Sendspin)
        {
            ...
            if (_sendspinPlayer != null)
            {
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        _logger.LogDebug(ex, "Failed to exit external source while switching away from Sendspin mode.");
                    }
        ...
        if (OutputMode == PlaybackOutputMode.Local)
        {
            await _musicAssistantEventHub.StopAsync(cancellationToken);
            ResetQueueState();
            _logger.LogDebug("Switched to Local. Event hub stopped and queue state reset.");
            return;
        }
    ...
    {
        if (OutputMode == PlaybackOutputMode.Local || string.IsNullOrWhiteSpace(ActivePlayerId))
        {
            ResetQueueState();
            return;
        }
    ...
    public async Task PlayQueueIndexAsync(int index, CancellationToken cancellationToken = default)
    {
        ...
        if (OutputMode == PlaybackOutputMode.Sendspin)
        {
    ...
    {
        if (OutputMode == PlaybackOutputMode.Sendspin && _isHybridLocalPlaybackActive && _localPlayer != null)
        {
    ...
    {
        if (OutputMode == PlaybackOutputMode.Sendspin && _currentQueueItems.Count > 0)
        {
    ...
    public async Task SetDontStopTheMusicAsync(bool enabled, CancellationToken cancellationToken = default)
    {
        if (OutputMode == PlaybackOutputMode.Local || string.IsNullOrWhiteSpace(ActivePlayerId))
        {
            _logger.LogWarning("SetDontStopTheMusic ignored in Local mode.");
            return;
        }
    ...
    {
        if (OutputMode == PlaybackOutputMode.Sendspin && _isHybridLocalPlaybackActive)
        {
    ...
    private async Task PlayMediaLocalAsync(IReadOnlyList<MediaItem> items, QueueOption queueOption, bool shuffle = false)
    {
        if (OutputMode != PlaybackOutputMode.Local)
        {
            _logger.LogWarning("PlayMediaLocal ignored because OutputMode is not Local.");
            return;
        }
    ...
    private bool IsSendspinAvailable =>
        OutputMode == PlaybackOutputMode.Sendspin
